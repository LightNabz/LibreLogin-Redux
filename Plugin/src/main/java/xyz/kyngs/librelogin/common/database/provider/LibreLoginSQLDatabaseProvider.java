...
public abstract class LibreLoginSQLDatabaseProvider extends AuthenticDatabaseProvider<SQLDatabaseConnector> {
    ...
    private User getUserFromResult(ResultSet rs) throws SQLException {
        if (rs.next()) {
            ...
            Long lastLocationTimestamp = rs.getLong("last_location_timestamp");
            ...
            if (lastWorld != null) {
                user.setLastWorld(lastWorld);
            }
            if (!rs.wasNull() && lastLocationTimestamp != null) {
                user.setLastLocationTimestamp(new java.sql.Timestamp(lastLocationTimestamp));
            }
    ...
    public void insertUser(User user) {
        ...
        connector.runQuery(connection -> {
            var ps = connection.prepareStatement("INSERT INTO librepremium_data(uuid, premium_uuid, hashed_password, salt, algo, last_nickname, joined, last_seen, secret, ip, last_authentication, last_server, email, last_x, last_y, last_z, last_world, last_location_timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    public void insertUsers(Collection<User> users) {
        plugin.reportMainThread();
        connector.runQuery(connection -> {
            var ps = connection.prepareStatement("INSERT " + getIgnoreSyntax() + " INTO librepremium_data(uuid, premium_uuid, hashed_password, salt, algo, last_nickname, joined, last_seen, secret, ip, last_authentication, last_server, email, last_x, last_y, last_z, last_world, last_location_timestamp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)" + getIgnoreSuffix());
    ...
    private void insertToStatement(PreparedStatement ps, User user) throws SQLException {
        ...
        ps.setObject(14, Double.isNaN(user.getLastX()) ? null : user.getLastX());
        ps.setObject(15, Double.isNaN(user.getLastY()) ? null : user.getLastY());
        ps.setObject(16, Double.isNaN(user.getLastZ()) ? null : user.getLastZ());
        ps.setString(17, user.getLastWorld());
        ps.setObject(18, user.getLastLocationTimestamp() == null ? null : user.getLastLocationTimestamp().getTime());
    }
    ...
    public void updateUser(User user) {
        plugin.reportMainThread();
        connector.runQuery(connection -> {
            var ps = connection.prepareStatement("UPDATE librepremium_data SET premium_uuid=?, hashed_password=?, salt=?, algo=?, last_nickname=?, joined=?, last_seen=?, secret=?, ip=?, last_authentication=?, last_server=?, email=?, last_x=?, last_y=?, last_z=?, last_world=?, last_location_timestamp=? WHERE uuid=?");
            ...
            ps.setObject(13, Double.isNaN(user.getLastX()) ? null : user.getLastX());
            ps.setObject(14, Double.isNaN(user.getLastY()) ? null : user.getLastY());
            ps.setObject(15, Double.isNaN(user.getLastZ()) ? null : user.getLastZ());
            ps.setString(16, user.getLastWorld());
            ps.setObject(17, user.getLastLocationTimestamp() == null ? null : user.getLastLocationTimestamp().getTime());
            ps.setString(18, user.getUuid().toString());
            ps.executeUpdate();
        });
    }
    ...
    public void validateSchema() {
        connector.runQuery(connection -> {
            ...
            if (!columns.contains("last_x")) {
                connection.prepareStatement("ALTER TABLE librepremium_data ADD COLUMN last_x DOUBLE NULL DEFAULT NULL").executeUpdate();
            }
            if (!columns.contains("last_y")) {
                connection.prepareStatement("ALTER TABLE librepremium_data ADD COLUMN last_y DOUBLE NULL DEFAULT NULL").executeUpdate();
            }
            if (!columns.contains("last_z")) {
                connection.prepareStatement("ALTER TABLE librepremium_data ADD COLUMN last_z DOUBLE NULL DEFAULT NULL").executeUpdate();
            }
            if (!columns.contains("last_world")) {
                connection.prepareStatement("ALTER TABLE librepremium_data ADD COLUMN last_world VARCHAR(255) NULL DEFAULT NULL").executeUpdate();
            }
            if (!columns.contains("last_location_timestamp")) {
                connection.prepareStatement("ALTER TABLE librepremium_data ADD COLUMN last_location_timestamp BIGINT NULL DEFAULT NULL").executeUpdate();
            }
