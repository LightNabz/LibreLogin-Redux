...
public class ConfigurationKeys {
    ...
    public static final ConfigurationKey<Boolean> REMEMBER_LOCATION = new ConfigurationKey<>(
            "remember-location.enabled",
            true,
            "Should we remember the exact location where players logged off and restore them there on login? Set to false to disable this feature.",
            ConfigurateHelper::getBoolean
    );

    public static final ConfigurationKey<List<String>> REMEMBER_LOCATION_WORLDS = new ConfigurationKey<>(
            "remember-location.allowed-worlds",
            List.of("world", "world_nether", "world_the_end"),
            """
                    The worlds where player locations should be remembered and restored. Limbo is always excluded.
                    """,
            ConfigurateHelper::getStringList
    );

    public static final ConfigurationKey<Integer> REMEMBER_LOCATION_MAX_AGE_DAYS = new ConfigurationKey<>(
            "remember-location.max-age-days",
            -1,
            "Maximum age in days for a saved location to be restored. Set to negative to disable age checking. Set to 0 to only restore same-day locations.",
            ConfigurateHelper::getInt
    );

    public static final ConfigurationKey<Boolean> REMEMBER_LOCATION_SAFETY_CHECKS = new ConfigurationKey<>(
            "remember-location.safety-checks",
            false,
            """
                    Should we perform safety checks before restoring a location? This includes checking if the block is safe to spawn on.
                    Disabled by default for performance reasons.
                    """,
            ConfigurateHelper::getBoolean
    );

    public static final ConfigurationKey<Boolean> REMEMBER_LOCATION_CREATE_FLOODGATE = new ConfigurationKey<>(
            "remember-location.create-floodgate-user",
            false,
            "If true, a database record will be created for Floodgate players when saving locations so their last location can be restored later. " +
                    "Requires remember-location.enabled to be true.",
            ConfigurateHelper::getBoolean
    );
}
